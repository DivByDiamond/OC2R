...
- **Terminal**: XTRESTORE of DECCOLM (`CSI ? 3 r`/`CSI ? 3 u`) restored the flag but never resized the buffer, leaving the terminal at the wrong column count after a save/toggle/restore cycle; it now resizes to match the restored flag, mirroring DECSET/DECRST's DECCOLM action
