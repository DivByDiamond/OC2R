...
### 44.3 Double-sized characters (ESC #3/#4/#6/#7) — большой, два PR
...
- [ ] PR-A: парсинг (`handleHash` знает только `'8'`/DECALN) + per-line атрибут
      ...
      Семантика — xterm-style (print из текущего курсора до EOL), не DEC self/relative.
      Рендер: глиф 2× по вертикали (верх — верхняя половина увеличенного, низ — нижняя).
- [ ] PR-B: double-width (`# 6`, `# 7`): 2 ячейки на символ → ломает autowrap (pending wrap
