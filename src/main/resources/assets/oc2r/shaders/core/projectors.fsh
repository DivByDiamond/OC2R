...
// Projector 3
uniform sampler2D ProjectorColor3;
uniform sampler2D ProjectorDepth3;
uniform mat4 ProjectorCamera3;

// Projector 4
uniform sampler2D ProjectorColor4;
uniform sampler2D ProjectorDepth4;
uniform mat4 ProjectorCamera4;

// Projector 5
uniform sampler2D ProjectorColor5;
uniform sampler2D ProjectorDepth5;
uniform mat4 ProjectorCamera5;

// Projector 6
uniform sampler2D ProjectorColor6;
uniform sampler2D ProjectorDepth6;
uniform mat4 ProjectorCamera6;

// Projector 7
uniform sampler2D ProjectorColor7;
uniform sampler2D ProjectorDepth7;
uniform mat4 ProjectorCamera7;
...
void main() {
    ...
    if (Count > 3 && getProjectorColor(worldPos, worldNormal, ProjectorCamera3, ProjectorColor3, ProjectorDepth3, color)) {
        colorAcc += color;
        accCount += 1;
    }
    if (Count > 4 && getProjectorColor(worldPos, worldNormal, ProjectorCamera4, ProjectorColor4, ProjectorDepth4, color)) {
        colorAcc += color;
        accCount += 1;
    }
    if (Count > 5 && getProjectorColor(worldPos, worldNormal, ProjectorCamera5, ProjectorColor5, ProjectorDepth5, color)) {
        colorAcc += color;
        accCount += 1;
    }
    if (Count > 6 && getProjectorColor(worldPos, worldNormal, ProjectorCamera6, ProjectorColor6, ProjectorDepth6, color)) {
        colorAcc += color;
        accCount += 1;
    }
    if (Count > 7 && getProjectorColor(worldPos, worldNormal, ProjectorCamera7, ProjectorColor7, ProjectorDepth7, color)) {
        colorAcc += color;
        accCount += 1;
    }
