...
final class ProjectorVideoEncoder {
    ...
    boolean hasChangesOrNeedsIDR(final ProjectorDevice device) {
        return device.hasChanges() || needsIDR;
    }

    @Nullable
    ByteBuffer encodeFrame(final ProjectorDevice device, final Picture picture) {
