...
public final class ComputerBlockEntity extends ModBlockEntity implements TerminalUserProvider, TickableBlockEntity, ICaptureInputStateStorage {
    public static final int MEMORY_SLOTS = 4, HARD_DRIVE_SLOTS = 4, FLASH_MEMORY_SLOTS = 1, CARD_SLOTS = 4, CPU_SLOTS = 1;
    ...
    @Override public boolean getCaptureInputState() { return captureInputState; }
    @Override public void setCaptureInputState(boolean value) { this.captureInputState = value; }
    public void start() { if (level != null && !level.isClientSide()) virtualMachine.start(); }
    public void stop() { if (level != null && !level.isClientSide()) virtualMachine.stop(); }
    public void openTerminalScreen(final ServerPlayer player) { ComputerTerminalContainer.createServer(this, energy, virtualMachine.busController, player); }
    public void openInventoryScreen(final ServerPlayer player) { ComputerInventoryContainer.createServer(this, energy, virtualMachine.busController, player); }
    ...
    @Override public Iterable<Player> getTerminalUsers() { return terminalUsers; }
    public void handleNeighborChanged() { if (level != null && !level.isClientSide()) virtualMachine.busController.scheduleBusScan(); }
    public void onAfterDeviceScan(final AfterDeviceScanEvent event) { if (event.didDevicesChange()) level.invalidateCapabilities(getBlockPos()); }
    ...
    public UUID getDeviceId() { return busElement.deviceId; }
    public boolean isContraptionVirtualClone() { return ComputerContraptionHandler.isContraptionVirtualClone(this); }
    @Nullable public ComputerBlockEntity getPrimaryForContraptionRendering() { return ComputerContraptionHandler.getPrimaryForContraptionRendering(this); }
    @Override public void clientTick() { terminal.clientTick(); ComputerContraptionHandler.registerInClientRegistry(this); }
    @Override public void onChunkUnloaded() { super.onChunkUnloaded(); ComputerContraptionHandler.unregisterFromClientRegistry(this); }
    @Override public void setRemoved() { super.setRemoved(); ComputerContraptionHandler.unregisterFromClientRegistry(this); }
    @Override public void serverTick() {
        if (level == null) return;
        if (!hasAddedOwnDevices) { hasAddedOwnDevices = true; busElement.addOwnDevices(); }
        if (isNeighborUpdateScheduled) { isNeighborUpdateScheduled = false; level.updateNeighborsAt(getBlockPos(), getBlockState().getBlock()); }
        chunk = level.getChunkAt(getBlockPos());
        virtualMachine.tick();
    }

    @Override protected void applyImplicitComponents(final DataComponentInput componentInput) {
        super.applyImplicitComponents(componentInput);
        var container = componentInput.get(DataComponents.RESTRICTED_CONTAINER);
        if (container != null) { deviceItems.loadItems(getLevel().registryAccess(), container); }
        else {
    ...
    @Override protected void collectImplicitComponents(final DataComponentMap.Builder components) {
    ...
    @Override public void handleUpdateTag(final CompoundTag tag, HolderLookup.Provider registries) {
        super.handleUpdateTag(tag, registries);
        ComputerBlockEntityPersistence.handleUpdateTagClient(this, tag, registries);
    }

    @Override protected void saveAdditional(final CompoundTag tag, HolderLookup.Provider registries) {
        super.saveAdditional(tag, registries);
        ComputerBlockEntityPersistence.saveAdditional(this, tag, registries);
    }

    @Override public void loadAdditional(final CompoundTag tag, HolderLookup.Provider registries) {
    ...
    @Override protected void loadClient() { super.loadClient(); terminal.setDisplayOnly(true); ComputerContraptionHandler.registerInClientRegistry(this); }
    @Override protected void loadServer() { super.loadServer(); assert level != null; virtualMachine.state.builtinDevices.rtcMinecraft.setLevel(level); }
    @Override protected void unloadServer(final boolean isRemove) {
        super.unloadServer(isRemove);
        if (isRemove) virtualMachine.stop(); else virtualMachine.suspend();
