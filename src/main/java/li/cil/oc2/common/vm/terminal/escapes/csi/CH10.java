    ...
    public void execute(final int[] args, final int argsCount, final CSIState state) {
        if (state.hash) { // XTPUSHCOLORS
            ...
            if (terminal.currentPrivateModeState.isAltBufferEnabled()) {
                ...
                Arrays.fill(terminal.altColors, endIndex, endIndex + chars + 1, TerminalColors.DEFAULT_COLORS.Copy());
                Arrays.fill(terminal.altColorsBackground, endIndex, endIndex + chars + 1, c.Copy());
                Arrays.fill(terminal.altStyles, endIndex, endIndex + chars + 1, TerminalColors.DEFAULT_STYLE);
            } else {
                ...
                Arrays.fill(terminal.colors, endIndex, endIndex + chars + 1, TerminalColors.DEFAULT_COLORS.Copy());
                Arrays.fill(terminal.colorsBackground, endIndex, endIndex + chars + 1, c.Copy());
                Arrays.fill(terminal.styles, endIndex, endIndex + chars + 1, TerminalColors.DEFAULT_STYLE);
            }
