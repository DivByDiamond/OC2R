...
public final class Allocator {
     ...
     *
     * <p>Claimed memory <em>must</em> be returned using {@link #freeMemory(UUID)} to prevent leaks.
     *
     ...
     *
     * <p>Using this if there was no memory claimed with ths handle or if the handle has already
     * been freed does nothing.
     *
    ...
    public static void resetAndCheckLeaks() {
        if (allocated > 0) {
            for (final Allocation allocation : ALLOCATIONS.values()) {
                // Skip first three: Allocator::claimMemory, Allocation::new,
                // Throwable::getStacktrace
                LOGGER.error(
                        Arrays.stream(allocation.stacktrace)
                                .skip(3)
    ...
    private static boolean checkArgs(final UUID handle, final int size) {
        if (ALLOCATIONS.containsKey(handle)) {
            throw new IllegalStateException(
                    "Handle is already in use. It must be freed before it can be reused.");
        }
        if (size < 0) {
