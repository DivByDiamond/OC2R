...
public class Terminal {
    ...
    public static final class Renderer implements RendererModel, RendererView {
        ...
        public void renderBuffer(final PoseStack stack, final Matrix4f projectionMatrix, boolean renderingToBlock) {
            ...
            // The font atlas is RGBA with alpha=0 for empty pixels. Without
            // proper blend state the alpha channel is ignored, so the entire
            // 16x32 glyph cell (including the empty corners) overwrites the
            // block face — which on a computer/monitor block face means the
            // background quad covers the screen overlay and the text appears
            // either invisible (alpha=0 wins) or as a solid white rectangle.
            //
            // The GUI path works because GuiGraphics already sets up blend
            // state for us. The block-entity-renderer path (ComputerRenderer /
            // MonitorRenderer) calls RenderSystem.enableBlend() but does NOT
            // call defaultBlendFunc(), so we can inherit a stale blend func
            // from a previous render pass (e.g. lightning's ONE/ONE). Set it
            // explicitly here so the text actually composites correctly on
            // the block face.
            ...
            if (renderingToBlock) {
                // Sodium/Sable/Flywheel compatibility: instead of manually
                // computing RenderSystem.getModelViewMatrix() * stack (which
                // reads a cached field that Sable may have left stale by
                // modifying the model-view stack without calling
                // applyModelViewMatrix()), we push our pose onto the
                // RenderSystem model-view stack and call applyModelViewMatrix()
                // ourselves. This guarantees the cached matrix is current
                // before we read it back for the explicit drawWithShader call.
                RenderSystem.getModelViewStack().pushMatrix();
                RenderSystem.getModelViewStack().mul(stack.last().pose());
                RenderSystem.applyModelViewMatrix();

                // Read the freshly-updated matrices. This is the key fix:
                // after applyModelViewMatrix(), getModelViewMatrix() returns
                // the product of whatever was on the stack (including Sable's
                // camera setup) and our pose. Without applyModelViewMatrix(),
                // the cached field could be stale.
                ...
                for (final VertexBuffer line : lines) {
                    if (line != null && !line.isInvalid()) {
                        try {
                            ...
                            // Was System.out.println — invisible in production
                            // logs. Use Log4j so users can actually diagnose
                            // VBO draw failures.
                ...
                // GUI path — the caller passes an orthographic projection
                // matrix and a screen-space PoseStack. Use the explicit-matrix
                // drawWithShader variant as before.
            ...
            // Restore blend state to default so we don't bleed into subsequent
            // render passes (the BER framework expects to leave blend as it
        ...
        public void renderCursor(final PoseStack stack) {
            ...
            // Match the blend state set up in renderBuffer so the cursor
            // composites correctly on the block face too.
            RenderSystem.enableBlend();
            RenderSystem.defaultBlendFunc();
            RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
            ...
            // Push our cursor-local pose (which includes the block-face
            // alignment from the caller's PoseStack) onto the RenderSystem
            // model-view stack, same as renderBuffer does. This is needed
            // for Sodium/Sable compatibility — BufferUploader.drawWithShader
            // uses RenderSystem.getModelViewMatrix() internally, so the
            // cursor would be drawn at the wrong position without this.
            RenderSystem.getModelViewStack().pushMatrix();
            RenderSystem.getModelViewStack().mul(stack.last().pose());
            RenderSystem.applyModelViewMatrix();

            // Use an identity matrix for vertex positions — the pose is now
            // on the RenderSystem model-view stack.
