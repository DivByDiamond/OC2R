...
public final class BlockDeviceBusController extends CommonDeviceBusController {
    ...
    public BlockDeviceBusController(
    ...
    public void setDeviceContainersChanged() {
        ...
        for (final ChunkLocation location : busChunks) {
            location.tryGetLevel()
                    .ifPresent(level -> ChunkUtils.setLazyUnsaved(level, location.position()));
        }
    }
    ...
    protected void onAfterBusScan() {
        ...
        for (final DeviceBusElement element : getElements()) {
            if (element instanceof final BlockDeviceBusElement blockElement) {
                ...
                if (elementLevel != null) {
                    final ChunkLocation elementLocation =
                            ChunkLocation.of(elementLevel, elementPosition);
                    busChunks.add(elementLocation);
                    newTrackedChunks.add(elementLocation);
                    newTrackedChunks.add(
                            ChunkLocation.of(
                                    elementLevel, elementPosition.relative(Direction.NORTH)));
                    newTrackedChunks.add(
                            ChunkLocation.of(
                                    elementLevel, elementPosition.relative(Direction.EAST)));
                    newTrackedChunks.add(
                            ChunkLocation.of(
                                    elementLevel, elementPosition.relative(Direction.SOUTH)));
                    newTrackedChunks.add(
                            ChunkLocation.of(
                                    elementLevel, elementPosition.relative(Direction.WEST)));
                }
            }
        }
        ...
        final ChunkLocation controllerChunkLocation =
                ChunkLocation.of(level, blockEntity.getBlockPos());
    ...
    private void addListeners(final Collection<ChunkLocation> trackedChunks) {
        for (final ChunkLocation trackedChunk : trackedChunks) {
            trackedChunk
                    ...
                    .ifPresent(
                            level -> {
                                ServerScheduler.subscribeOnLoad(
                                        level,
                                        trackedChunk.position(),
                                        onBusChunkLoadedStateChanged);
                                ServerScheduler.subscribeOnUnload(
                                        level,
                                        trackedChunk.position(),
                                        onBusChunkLoadedStateChanged);
                            });
    ...
    private void removeListeners(final Collection<ChunkLocation> trackedChunks) {
        for (final ChunkLocation trackedChunk : trackedChunks) {
            trackedChunk
                    ...
                    .ifPresent(
                            level -> {
                                ServerScheduler.unsubscribeOnLoad(
                                        level,
                                        trackedChunk.position(),
                                        onBusChunkLoadedStateChanged);
                                ServerScheduler.unsubscribeOnUnload(
                                        level,
                                        trackedChunk.position(),
                                        onBusChunkLoadedStateChanged);
                            });
