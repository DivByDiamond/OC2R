                ...
                new MenuProvider() {
                    ...
                    public Component getDisplayName() {
                        return Component.translatable(
                                computer.getBlockState().getBlock().getDescriptionId());
                    }

                    @Override
                    public AbstractContainerMenu createMenu(
                            final int id, final Inventory inventory, final Player player) {
                        return new ComputerInventoryContainer(
                                id, computer, player, createEnergyInfo(energy, busController));
                    }
                },
                computer.getBlockPos());
    }

    public static ComputerInventoryContainer createClient(
            final int id, final Inventory playerInventory, final FriendlyByteBuf data) {
        ...
        if (blockEntity instanceof final ComputerBlockEntity computer) {
            return new ComputerInventoryContainer(
                    id, computer, playerInventory.player, createClientEnergyInfo());
        }

        throw new IllegalArgumentException();
    }

    private ComputerInventoryContainer(
            final int id,
            final ComputerBlockEntity computer,
            final Player player,
        ...
        handlers.getItemHandler(DeviceTypes.FLASH_MEMORY)
                .ifPresent(
                        itemHandler -> {
                            if (itemHandler.getSlots() > 0) {
                                addSlot(
                                        new DeviceTypeSlotItemHandler(
                                                itemHandler, DeviceTypes.FLASH_MEMORY, 0, 64, 78));
                            }
                        });

        handlers.getItemHandler(DeviceTypes.MEMORY)
                .ifPresent(
                        itemHandler -> {
        ...
        handlers.getItemHandler(DeviceTypes.HARD_DRIVE)
                .ifPresent(
                        itemHandler -> {
                            for (int slot = 0; slot < itemHandler.getSlots(); slot++) {
                                addSlot(
        ...
        handlers.getItemHandler(DeviceTypes.CPU)
                .ifPresent(
                        itemHandler -> {
                            if (itemHandler.getSlots() > 0) {
                                addSlot(
                                        new DeviceTypeSlotItemHandler(
                                                itemHandler, DeviceTypes.CPU, 0, 64, 52));
                            }
                        });

        handlers.getItemHandler(DeviceTypes.GPU)
                .ifPresent(
                        itemHandler -> {
                            if (itemHandler.getSlots() > 0) {
                                addSlot(
                                        new DeviceTypeSlotItemHandler(
                                                itemHandler, DeviceTypes.GPU, 0, 82, 52));
                            }
                        });
