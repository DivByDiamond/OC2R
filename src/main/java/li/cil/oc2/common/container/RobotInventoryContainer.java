        ...
        player.openMenu(
                new MenuProvider() {
                    ...
                    public AbstractContainerMenu createMenu(
                        ...
                        return new RobotInventoryContainer(
                                id, robot, player, createEnergyInfo(energy, busController));
                    }
                },
                b -> b.writeVarInt(robot.getId()));
    }

    public static RobotInventoryContainer createClient(
            final int id, final Inventory inventory, final FriendlyByteBuf data) {
        ...
        if (entity instanceof final Robot robot) {
            return new RobotInventoryContainer(
                    id, robot, inventory.player, createClientEnergyInfo());
        }

        throw new IllegalArgumentException();
    }

    private RobotInventoryContainer(
            final int id,
            final Robot robot,
            final Player player,
                            ...
                            if (itemHandler.getSlots() > 0) {
                                addSlot(
                                        new DeviceTypeSlotItemHandler(
                                                itemHandler, DeviceTypes.FLASH_MEMORY, 0, 34, 78));
                            }
                        });
        ...
        handlers.getItemHandler(DeviceTypes.ROBOT_MODULE)
                .ifPresent(
                        itemHandler -> {
                            for (int slot = 0; slot < itemHandler.getSlots(); slot++) {
                                addSlot(
                ...
                .ifPresent(
                        itemHandler -> {
                            if (itemHandler.getSlots() > 0) {
                                addSlot(
                                        new DeviceTypeSlotItemHandler(
                                                itemHandler, DeviceTypes.CPU, 0, 34, 52));
                            }
                        });
