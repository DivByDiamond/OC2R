...
final class BlobChannelManager {
    ...
    private static final Map<UUID, CompletableFuture<FileChannel>> PENDING_OPERATIONS =
    ...
    static CompletableFuture<FileChannel> openAsync(final UUID handle) {
        ...
        return PENDING_OPERATIONS.computeIfAbsent(
                handle,
                h ->
                        AsyncUtils.runAsync(
                                () -> {
    ...
    static CompletableFuture<Void> closeAsync(final UUID handle) {
        ...
        try {
            ...
            LOGGER.trace("config not yet initialized", ignored);
        }
