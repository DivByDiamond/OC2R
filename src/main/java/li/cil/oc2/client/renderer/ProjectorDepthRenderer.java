...
public final class ProjectorDepthRenderer {
    ...
    static final DepthOnlyRenderTarget[] PROJECTOR_DEPTH_TARGETS = new DepthOnlyRenderTarget[ModShaders.MAX_PROJECTORS];
    static final DynamicTexture[] PROJECTOR_COLOR_TARGETS = new DynamicTexture[ModShaders.MAX_PROJECTORS];
    ...
    public static void renderProjectors(final RenderLevelStageEvent event) {
        ...
        try {
            ...
            DepthBufferStage.renderProjectorDepths(minecraft, level, event.getPartialTick(), projectorCount);
            ColorCompositingStage.renderProjectorColors(minecraft, event.getPoseStack().last().pose(), event.getProjectionMatrix(), projectorCount);
        } finally {
