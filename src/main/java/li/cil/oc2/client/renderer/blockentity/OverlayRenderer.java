...
final class OverlayRenderer {
    static final Material TEXTURE_POWER = new Material(InventoryMenu.BLOCK_ATLAS, ComputerRenderer.OVERLAY_POWER_LOCATION);
    static final Material TEXTURE_STATUS = new Material(InventoryMenu.BLOCK_ATLAS, ComputerRenderer.OVERLAY_STATUS_LOCATION);
    static final Material TEXTURE_TERMINAL = new Material(InventoryMenu.BLOCK_ATLAS, ComputerRenderer.OVERLAY_TERMINAL_LOCATION);

    static void renderStatus(final Matrix4f matrix, final MultiBufferSource bufferSource) {
        renderStatus(matrix, bufferSource, 0);
    }

    static void renderStatus(final Matrix4f matrix, final MultiBufferSource bufferSource, final int frequency) {
