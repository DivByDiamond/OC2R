...
public final class ComputerRenderer implements BlockEntityRenderer<ComputerBlockEntity> {
    ...
    static final Cache<Terminal, RendererView> rendererViews = CacheBuilder.newBuilder()
    ...
    public void render(final ComputerBlockEntity computer, final float partialTicks, final PoseStack stack, final MultiBufferSource bufferSource, final int light, final int overlay) {
        ...
        if (terminalSource.getVirtualMachine().isRunning()) {
            terminalTextRenderer.renderTerminal(stack, bufferSource, terminalSource.getTerminal(), distanceToCamera);
        } else {
            terminalTextRenderer.renderStatusText(stack, bufferSource, distanceToCamera, terminalSource.getVirtualMachine().getError());
        }
    ...
    static void handleNoLongerRendering(final RemovalNotification<Terminal, RendererView> notification) {
