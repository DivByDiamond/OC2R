...
final class TerminalTextRenderer {
    ...
    void renderTerminal(final PoseStack stack, final MultiBufferSource bufferSource, final Terminal terminal, final double distanceToCamera) {
        if (distanceToCamera < 6f) {
            ...
            try {
                ComputerRenderer.rendererViews.get(terminal, terminal::getRenderer).render(stack, RenderSystem.getProjectionMatrix(), true);
            ...
            OverlayRenderer.renderQuad(matrix, OverlayRenderer.TEXTURE_TERMINAL.buffer(bufferSource, ModRenderType::getUnlitBlock));

            stack.popPose();
        }
    }

    void renderStatusText(final PoseStack stack, final MultiBufferSource bufferSource, final double distanceToCamera, final Component bootError) {
    ...
    private void drawText(final MultiBufferSource bufferSource, final PoseStack stack, final Component text) {
    ...
    private void draw(final MultiBufferSource bufferSource, final Font font, final PoseStack stack, final Component text, final float x) {
        font.drawInBatch(text, x, (float) 0, 15610658, false, stack.last().pose(), bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
    }

    private void draw(final MultiBufferSource bufferSource, final Font font, final PoseStack stack, final String text, final float y) {
